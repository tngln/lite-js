...
let keymap = (() => {
  const modkeys: Record<string, boolean> = {}
  const map: Record<string, string[]> = {}
  const reverse_map: Record<string, string> = {}

  const modkey_map: Record<string, string> = {
    ["left ctrl"]:   "ctrl",
    ["right ctrl"]:  "ctrl",
    ["left shift"]:  "shift",
    ["right shift"]: "shift",
  ...
  const modkeys_order = ["ctrl", "alt", "altgr", "shift"]

  function key_to_stroke(k: string) {
    let stroke = ""
    for (let i = 1; i <= modkeys_order.length - 1; i++) {
      const mk = modkeys_order[i]
      if (modkeys[mk]) {
        stroke = stroke + mk + "+"
      }
    }
  ...
  function add(add_map: Record<string, string | string[]>, overwrite?: boolean) {
    for (const stroke in add_map) {
      let commands = add_map[stroke]
      ...
      if (overwrite) {
        ...
        for (let i = commands.length - 1; i >= 0; i--) {
          map[stroke].unshift(commands[i])
        }
      }
      for (let i = 0; i < commands.length; i++) {
        const cmd = commands[i]
        reverse_map[cmd] = stroke
      }
    }
  }

  function get_binding(cmd: string): string | undefined {
    return reverse_map[cmd]
  }

  function on_key_pressed(k: string): boolean {
    const mk = modkey_map[k]
    if (mk) {
      modkeys[mk] = true
      ...
      const stroke = key_to_stroke(k)
      const commands = map[stroke]
      if (commands) {
        for (let i = 1; i <= commands.length - 1; i++) {
          const cmd = commands[i]
          const performed = command.perform(cmd)
          if (performed) break
        }
        return true
      }
  ...
  function on_key_released(k: string) {
    const mk = modkey_map[k]
    if (mk) {
      modkeys[mk] = false
    }
  ...
  return { modkeys, map, reverse_map, add, get_binding, on_key_pressed, on_key_released }
})()
